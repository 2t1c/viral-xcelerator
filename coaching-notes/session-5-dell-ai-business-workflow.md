...
## Diagnosis
Media is **not** the bottleneck. The real time sinks are **research + the writing back-and-forth** (body > hook). Tonality is already handled by the prompts; the editing pattern is transitions + killing AI's choppy sentences.

## Key fixes
1. **Fix idea + angle at the very start** — get those right and the whole thread is easier to make perform.
2. **Level up the idea source** beyond scrolling saturated IG bean pages:
   - **XPro** — search "AI," get an abundance of already-proven ideas.
   - **A dedicated YouTube account** (alt email) subscribed only to AI content — within ~30 min the feed is gold; YouTube is searchable + gives transcripts.
   - **Podcasts (e.g. Diary of a CEO):** one episode = a week+ of content. Extract sections with Claude/Gemini (each theme = a post), clip the matching snippet for video. Highly scalable.
   - Don't feel boxed into "informative/educational" AI content — huge format diversity (prompts, demos, news, single-article → post in ~5 min).
3. **Separate writing from fact-checking via the outline.** Fact-check the *outline* (pure info, no fluff); if the outline is correct, the post built from it is correct. Then stick to that info and "add the wrapper." Avoids the dangerous AI fact-check loop (longer context → more hallucination).
4. **Use a master prompt** that fixes recurring AI errors in one shot, so you start editing from a "V2," not a raw V1.
...
## Three takeaways
1. Study AI creators closely — replicate (and beat) them.
2. Leverage QRT to pull engagement from existing posts.
3. Upgrade the idea source (XPro, YouTube account, podcasts) — lean into podcast → long-form + clip as the highest-leverage, most scalable play.
